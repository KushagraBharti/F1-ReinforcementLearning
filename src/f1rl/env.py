...
class MonzaEnv(gym.Env[np.ndarray, int]):
    ...
    def reset(
        ...
        *,
        ...
        if not reset_options and self.curriculum.enabled:
            reset_options = self.curriculum_sampler.sample_options(seed)
        obs, info = self.sim.reset(seed=seed, options=reset_options)
