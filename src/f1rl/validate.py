...
def run_import_smoke() -> None:
    ...
    from f1rl.hardware_check import main as _hardware_check_main
    ...
    _ = _F1RaceEnv, _hardware_check_main, _run_benchmark, _run_manual, _run_rollout, _run_training
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--skip-hardware", action="store_true", help="Skip GPU hardware validation command.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.skip_hardware:
        run_command([python, "-m", "f1rl.hardware_check", "--skip-train-smoke"])
    ...
    if not args.skip_train:
        run_command([python, "-m", "f1rl.train", "--mode", "smoke", "--iterations", "1", "--device", "auto"])
        run_command([python, "-m", "f1rl.eval", "--checkpoint", "latest", "--headless", "--steps", "120"])
        run_command([python, "-m", "f1rl.benchmark", "--checkpoint", "latest", "--profile", "quick", "--no-clips"])
