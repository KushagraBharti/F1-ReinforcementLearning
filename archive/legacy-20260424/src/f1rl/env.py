...
class F1RaceEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def __init__(self, env_config: dict[str, Any] | None = None) -> None:
        ...
        self._heading_away_steps = 0
        self._wall_hugging_steps = 0
        self._positive_progress_total = 0
    ...
    def _goal_spawn_pose(self, goal_idx: int) -> tuple[tuple[float, float], float, int]:
        spawn_goal_idx = int(goal_idx) % self.track.num_goals
        start_midpoint = self.track.goal_midpoint(spawn_goal_idx)
        next_goal_idx = (spawn_goal_idx + 1) % self.track.num_goals
        next_midpoint = self.track.goal_midpoint(next_goal_idx)
        dx = float(next_midpoint[0] - start_midpoint[0])
        dy = float(next_midpoint[1] - start_midpoint[1])
        heading_deg = float(math.degrees(math.atan2(-dy, dx)))
        return (float(start_midpoint[0]), float(start_midpoint[1])), heading_deg, next_goal_idx
    ...
    def _resolve_start_pose(
        ...
        option_goal_idx = options.get("start_goal_idx") if options else None
        ...
        if options and "start_pos" in options:
            ...
            return start_pos, start_heading, 1

        if option_goal_idx is not None:
            start_pos, start_heading, next_goal_idx = self._goal_spawn_pose(int(option_goal_idx))
        elif self.config.spawn_goal_range is not None:
            start_idx, end_idx = self.config.spawn_goal_range
            sampled_goal_idx = int(self.np_random.integers(start_idx, end_idx + 1))
            start_pos, start_heading, next_goal_idx = self._goal_spawn_pose(sampled_goal_idx)
        else:
        ...
        if self.config.spawn_position_jitter_px > 0.0:
            ...
            start_pos = (float(start_pos[0] + jitter[0]), float(start_pos[1] + jitter[1]))
        ...
        return start_pos, start_heading, next_goal_idx
    ...
    def _build_info(self, collided: bool, progress_delta: int, terminated_reason: str | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "heading_away_steps": int(self._heading_away_steps),
            "wall_hugging_steps": int(self._wall_hugging_steps),
            "reward_components": dict(self.last_reward_components),
            "terminated_reason": terminated_reason,
        }

    def _missed_checkpoint_deadline(self) -> bool:
        if self.car.step_count <= self.config.spawn_grace_steps:
            return False
        for checkpoint_target, deadline_step in self.config.checkpoint_deadlines:
            if self.car.step_count >= int(deadline_step) and self._positive_progress_total < int(checkpoint_target):
                return True
        return False
    ...
    def _apply_scripted_warm_start(self) -> None:
        warm_start_steps = max(0, int(self.config.scripted_warm_start_steps))
        if warm_start_steps <= 0:
            return

        initial_car = replace(self.car)
        controller = ScriptedController(sensor_count=self.sensor_count, goals=self.track.goals)
        controller.reset()

        obs = self._compute_observation()
        info = self._build_info(collided=False, progress_delta=0)
    ...
    def reset(
        ...
        *,
        ...
        start_pos, start_heading, next_goal_idx = self._resolve_start_pose(options)
        self.car = new_car_state(start_pos=start_pos, heading_deg=start_heading)
        self.car.next_goal_idx = int(next_goal_idx)
        self._reset_episode_trackers()
        self._apply_scripted_warm_start()
        obs = self._compute_observation()
    ...
    def step(self, action: np.ndarray) -> tuple[np.ndarray, float, bool, bool, dict[str, Any]]:
        ...
        self.car, movement = apply_dynamics(
            self.car,
            clipped_action,
            self.config.dynamics,
            meters_per_world_unit=self.track.meters_per_world_unit,
        )
        ...
        if progress_delta > 0:
            ...
            self._positive_progress_total += progress_delta
        ...
        if self.car.step_count > self.config.spawn_grace_steps and abs(heading_error_norm) >= self.config.heading_away_threshold:
            self._heading_away_steps += 1
        else:
            self._heading_away_steps = 0
        ...
        obs = self._compute_observation()
        min_sensor_ratio = float(np.min(np.clip((obs[: self.sensor_count] + 1.0) * 0.5, 0.0, 1.0)))
        if self.car.step_count > self.config.spawn_grace_steps and min_sensor_ratio <= self.config.wall_hugging_distance_ratio_threshold:
            self._wall_hugging_steps += 1
        else:
            self._wall_hugging_steps = 0

        if self.config.heading_away_limit_steps > 0 and self._heading_away_steps >= self.config.heading_away_limit_steps:
            self.terminated = True
            terminated_reason = terminated_reason or "heading_away"

        if self.config.wall_hugging_limit_steps > 0 and self._wall_hugging_steps >= self.config.wall_hugging_limit_steps:
            self.terminated = True
            terminated_reason = terminated_reason or "wall_hugging"

        if self._missed_checkpoint_deadline():
            self.terminated = True
            terminated_reason = terminated_reason or "checkpoint_deadline"
